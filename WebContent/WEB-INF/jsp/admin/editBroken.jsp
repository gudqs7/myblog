...
<script type="text/javascript">
	var UEDITOR_HOME_URL = "ued";
</script>
<c:if test="${broken==null }"><c:set var="opr" value="新增"/><c:set var="action" value="add"/></c:if>
<c:if test="${broken!=null }"><c:set var="opr" value="修改"/><c:set var="action" value="update"/></c:if>
<title>碎言碎语${opr}页面</title>
</head>
<body>
	<form name="myform"  action="${pageContext.request.contextPath }/broken/${action}.do" method="post" enctype="multipart/form-data">
		...
		<table width='50%' cellpadding="7" cellspacing="0" border="1" align="center">
			...
			<tr>
				<th>内容</th>
				<td align='left'>
					<div style='display:none;' id='old'>${broken.content }</div>
			...
			<tr>
				<th>图片</th>
				<td>
					<c:if test="${broken!=null and broken.pic!=null }">
						<img width='200px' src="/pic/${broken.pic }" /><br/>
					</c:if>
...
<div align='center' style='margin:7px;'>
		...
		<input value="返回" type="button" onclick="location.href='${pageContext.request.contextPath}/broken/list.do'">
<div>
	</form>
<script type="text/javascript">
...
<script type="text/javascript">
		...
		//function getAllHtml(){
			...
			if(document.getElementById("content").value.length>1000) {alert("字数长度超过限制");return false};
			return true;
		}
